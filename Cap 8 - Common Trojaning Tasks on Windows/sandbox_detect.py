...
class Detector:
    ...
    def detect(self):
        ...
        firts_double_click = None
        double_click_threshold = 0.35

        max_double_clicks = 10
        max_keystrockes = random.randint(10, 25)
        ...
        while not detection_complete:
            ...
            if keypress_time is not None and previous_timestamp is not None:
                ...
                if elapsed <= double_click_threshold:
                    ...
                    if firts_double_click is None:
                        firts_double_click = time.time()
                    else:
                        if self.double_clicks >= max_double_clicks:
                            if (keypress_time - firts_double_click <=
                                (max_double_clicks*double_click_threshold)):
                                sys.exit(0)
                if (self.keystrokes >= max_keystrockes 
